...
if(isset($_GET['add']) || isset($_GET['edit'])){
    ...
    if(isset($_GET['edit'])){
        ...
        $description = ((isset($_POST['description']))?sanitize($_POST['description']):$product['descriptions']);
    ...
    if($_POST){
        ...
        if($photoCount > 0){
            for($i=0; $i<$photoCount; $i++){
                ...
                $dbpath .= '/EcomSite/images/products/'.$uploadName;
                if($mimeType != 'image'){
        ...
        if(!empty($errors)){
            ...
            $insertSql = "insert into products (`title`, `price`, `list_price`, `brand`, `categories`, `sizes`, `image`, `descriptions`) values
            ('$title', '$price', '$list_price', '$brand', '$category', '$sizes', '$dbpath', '$description') ";

            if(isset($_GET['edit'])){
                $insertSql = "update products set title = '$title', price = '$price', list_price = '$list_price', brand = '$brand', categories = '$category',
                sizes = '$sizes', image = '$dbpath', descriptions = '$description' where id = '$edit_id'";
            }
            //echo $insertSql; die();
            $db->query($insertSql);
            header('Location: products.php');
